...
keepSamples=NULL){
	...
	print(head(mcols(rowRanges(currvcf))[,'ALT']))
	print(head(S4Vectors::unlist(mcols(rowRanges(currvcf))[,'ALT'])))
	print(head(as.character(S4Vectors::unlist(mcols(rowRanges(currvcf))[,'ALT']))))
	...
	if(calculateFst){
		cat('calculating Fst...\n')
		wc.out<-MakeDiploidFSTMat(genos,rownames(genos),pops)
		res$Fst=wc.out$FST
